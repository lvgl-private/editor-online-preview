...
function createFileNode(basePath, itemPath, prefix) {
    ...
    if (isDirectory) {
        ...
        const children = items
            .map((item) =>
                createFileNode(basePath, path.join(itemPath, item), prefix)
            )
...
export function generateManifest(directoryPath, prefix, writeToFile = false) {
...
if (import.meta.url === `file://${process.argv[1]}`) {
    ...
    try {
        generateManifest(fullPath, prefix, true);
